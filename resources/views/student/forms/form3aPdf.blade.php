...
<body class="p-8 text-sm font-arial">
    ...
    <div class="flex flex-col border">
        ...
        <div class="flex items-center border-t">
            ...
            <div class="flex-1 p-2">{{ $form3a->protocol ?? $researchInfo->research_title ?? 'N/A' }}</div>
        </div>
        
        <!-- Version number/date -->
        <div class="flex items-center border-t">
            <div class="w-1/4 border-r border-black p-2">Version number/date</div>
            <div class="flex-1 p-2">{{ $form3a->version_no ?? 'N/A' }}</div>
        </div>
        ...
        <div class="flex items-stretch border-t">
            ...
            <div class="w-1/4 p-2 font-bold">{{ $form3a->study_site ?? 'N/A' }}</div>
        </div>
        ...
        <div class="flex border-t text-xs">
            ...
            <div class="w-1/2 border-r flex flex-col">
                <div class="flex border-b">
                    ...
                    <div class="w-1/4 p-2">{{ $form3a->pi_name ?? $principalInvestigator ?? 'N/A' }}</div>
                </div>
                <div class="flex">
                    <div class="w-1/4 border-r ml-2">Co-Investigator/s (if any)</div>
                    <div class="w-1/2 p-2">{{ $form3a->coi_name ?? 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="w-1/2 flex">
                ...
                <div class="flex flex-col w-3/4">
                    <div class="flex border-b">
                        ...
                        <div class="w-2/3 p-2">{{ $form3a->tel_no ?? 'N/A' }}</div>
                    </div>
                    <div class="flex border-b">
                        <div class="w-1/3 border-r p-2">Mobile No:</div>
                        <div class="w-2/3 p-2">{{ $form3a->contact_no ?? 'N/A' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-1/3 border-r p-2">Email:</div>
                        <div class="w-2/3 p-2">{{ $form3a->pi_email ?? 'N/A' }}</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="flex items-center border-t">
            ...
            <p class="text-xs text-l mb-2">{{ $form3a->investigator_institution ?? 'N/A' }}</p>
        </div>
        <div class="flex items-center border-t">
            <div class="px-2 w-1/4 border-r">
                <p class="text-xs font-bold text-l mb-2">Address of Institution</p>
            </div>
            <p class="text-xs text-l mb-2">{{ $form3a->institution_address ?? 'N/A' }}</p>
        </div>
    </div>
    ...
    <div class="flex flex-col border">
        ...
        <div class="flex items-stretch border-t">
            <div class="px-2 w-2/3 border-r">
                <p class="text-xs text-l mb-2">{{ $form3a->recommendations ?? 'N/A' }}</p>
            </div>
            <div class="px-2 w-2/3 border-r">
                <p class="text-xs text-l mb-2">{{ $form3a->research_response ?? 'N/A' }}</p>
            </div>
            <div class="px-2 w-1/4">
                <p class="text-xs text-l mb-2">{{ $form3a->section_page_number ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
