                        ...
                        <td>
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-medium">{{ $assignReviewer->researchInformation?->research_title }}</span>
                                </div>

                                {{-- 🔹 RE-ASSIGNMENT ALERT BADGE WITH FULL NAME 🔹 --}}
                                @if($assignReviewer->protocol && $assignReviewer->protocol->evaluatedReviews->where('status', 'Declined')->count() > 0)
                                    <div class="ml-6 mt-1">
                                        @foreach($assignReviewer->protocol->evaluatedReviews->where('status', 'Declined') as $declined)
                                            <span class="inline-block bg-red-100 text-red-700 text-[10px] font-bold px-2 py-0.5 rounded border border-red-300 uppercase">
                                                Declined by: 
                                                @if($declined->reviewer)
                                                    {{ $declined->reviewer->user_Fname }} {{ $declined->reviewer->user_Lname }}
                                                @else
                                                    Unknown Reviewer
                                                @endif
                                            </span>
