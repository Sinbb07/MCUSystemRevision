...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $validate = $request->validate([
            ...
            'research_type' => ['required', 'string', Rule::in([
                'MCU Undergraduate Student',
                'MCU Graduate Student',
                'MCU Faculty',
                'MCU Non-Teaching Staff',
                'MCU Administrative Staff',
                'Non-MCU Researcher',
            ])],
            ...
            'research_Endorsement' => ['required', 'file', 'mimes:pdf,jpg,jpeg,png,docx', 'max:2048'],
            'research_Receipts' => ['required', 'file', 'mimes:pdf,jpg,jpeg,png,docx', 'max:2048'],
        ]);
        ...
        ProcessMonitoring::create([
            ...
            if ($request->hasFile('research_Endorsement')) {
                $fileData = $request->file('research_Endorsement');
                $file = is_array($fileData) ? reset($fileData) : $fileData;

                if (! $file || ! $file->isValid()) {
                    Log::error('Endorsement upload invalid', [
                        'user_id' => $user->user_ID,
                        'hasFile' => $request->hasFile('research_Endorsement'),
                        'fileData' => is_object($file) ? get_class($file) : gettype($file),
                    ]);

                    throw new \Exception('Endorsement file is invalid or missing.');
                }
                ...
                Log::debug('Storing endorsement upload', [
                    'user_id' => $user->user_ID,
                    'field' => 'research_Endorsement',
                ...
                $sourcePath = $realPath ?: $pathName;
                if (! $sourcePath) {
                    Log::error('Upload file path is empty', ['user_id' => $user->user_ID, 'field' => 'research_Endorsement']);
                    throw new \Exception('Endorsement file path is empty.');
                }

                $stream = fopen($sourcePath, 'rb');
                if (! $stream) {
                    Log::error('Failed to open endorsement file stream', ['user_id' => $user->user_ID, 'source_path' => $sourcePath]);
                ...
                if ($research_Endorsement === false) {
                    Log::error('Failed to store endorsement file', ['user_id' => $user->user_ID, 'filename' => $filename]);
                    throw new \Exception('Failed to store endorsement file: ' . $filename);
                }
            }
            ...
            if ($request->hasFile('research_Receipts')) {
                $fileData = $request->file('research_Receipts');
                $file = is_array($fileData) ? reset($fileData) : $fileData;

                if (! $file || ! $file->isValid()) {
                    Log::error('Receipt upload invalid', [
                        'user_id' => $user->user_ID,
                        'hasFile' => $request->hasFile('research_Receipts'),
                        'fileData' => is_object($file) ? get_class($file) : gettype($file),
                    ]);

                    throw new \Exception('Receipts file is invalid or missing.');
                }
                ...
                Log::debug('Storing receipt upload', [
                    'user_id' => $user->user_ID,
                    'field' => 'research_Receipts',
                    'original_name' => $file->getClientOriginalName(),
                    'extension' => $file->getClientOriginalExtension(),
                    'mime' => $file->getClientMimeType(),
                    'size' => $file->getSize(),
                ...
                $sourcePath = $realPath ?: $pathName;
                if (! $sourcePath) {
                    Log::error('Upload file path is empty', ['user_id' => $user->user_ID, 'field' => 'research_Receipts']);
                    throw new \Exception('Receipts file path is empty.');
                }

                $stream = fopen($sourcePath, 'rb');
                if (! $stream) {
                    Log::error('Failed to open receipt file stream', ['user_id' => $user->user_ID, 'source_path' => $sourcePath]);
                ...
                if ($research_Receipts === false) {
                    Log::error('Failed to store receipt file', ['user_id' => $user->user_ID, 'filename' => $filename]);
                    throw new \Exception('Failed to store receipt file: ' . $filename);
                }
            }
            ...
            ResearchInformation::create([
                ...
                'user_ID'               => $user->user_ID,
                'research_type'         => $validate['research_type'],
